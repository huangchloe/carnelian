...
export default function Home() {
  ...
  return (
    <main style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '80px 24px 60px' }}>
      <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px 32px', zIndex: 50, background: '#f7f5f1' }}>
      ...
      <div style={{ width: '100%', maxWidth: 640, textAlign: 'center', marginBottom: showResults ? 32 : 52, transition: 'margin 0.3s ease', marginTop: showResults ? 0 : 40 }}>
        {!showResults && (
          <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(2.6rem, 5vw, 4rem)', fontWeight: 400, lineHeight: 1.15, letterSpacing: '-0.01em', marginBottom: 16, color: '#1a1816' }}>
            The story behind<br /><em>everything</em> you love.
          </h1>
        )}
        {!showResults && (
          <p style={{ fontSize: 15, color: '#888480', lineHeight: 1.7, marginBottom: 0 }}>
      ...
      <p style={{ position: 'fixed', bottom: 20, fontSize: 11, color: '#c8c4bc', letterSpacing: '0.1em', textTransform: 'uppercase' }}>
